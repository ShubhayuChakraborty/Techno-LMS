...
export default function MemberBorrowsPage() {
  ...
  const active = useMemo(() => allBorrows.filter((b) => b.status !== "returned"), [allBorrows]);
  const history = useMemo(() => allBorrows.filter((b) => b.status === "returned"), [allBorrows]);
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            {active.length} active &middot; {history.length} returned
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {data.map((b) => {
            ...
            const isOverdue = b.status === "overdue" || (!b.returnedAt && days < 0);
            return (
              <div
                key={b.id}
                className="card"
                ...
                >
                  <div style={{ flex: 1 }}>
                    ...
                    >
                      ...
                      <span>
                        Due:{" "}
                        <strong style={{ color: isOverdue ? "var(--danger)" : "var(--body)" }}>
                  ...
                  >
                    <BorrowStatusBadge dueDate={b.dueDate} returnedAt={b.returnedAt} />
                    {!b.returnedAt && (
                      <div
                        style={{
                          fontSize: 12,
